...
- Pace: [src/lib/pace.ts](./src/lib/pace.ts)'s `getPace()` is a pure function (no component state, computed server-side and passed down as a prop — unlike the countdown, it doesn't need per-minute ticking, so there's no client hook for it) that answers "am I keeping up," not just "how much time is left." It compares actual topics done against the topics you'd expect to have done by now given how much of the whole prep window (subject creation → exam date) has elapsed, with a 15%-of-total buffer before calling it ahead/behind rather than flagging every small deviation. Returns `null` (and [PaceBadge](./src/components/subjects/pace-badge.tsx) renders nothing) whenever there's nothing useful to say: no exam date, no topics, already finished, or the exam's already passed. Shown on both the dashboard cards and the subject detail page.
